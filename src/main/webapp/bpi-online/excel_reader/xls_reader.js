	...
	if(!empty(output))
	{
		...
		for(var k in first_row) keys.push(k);
		 ...
		 if(keys.length>2)
		 {
			  ...
			  $(".errorFileFormat").show();
			$(".errorFileFormat").html("Excel should contains only 2 columns");  
			 return false;
		 }
		 if(empty(part_no_key) || empty(qty_key) || qty_key=='undefined' || part_no_key=='undefined')
		 {
			 ...
			 $(".errorFileFormat").show();
			$(".errorFileFormat").html("Excel should contains atleast 2 columns");  
			 return false;
		 }
		 else
		 {
...
function UploadExcelOrder( ) {
 ...
 {
		$(".errorFileFormat").show();
		  $(".errorFileFormat").html("Error Message: Please Upload the file in Excel Format.");
		  return false
 }
 file_name=f.name;
